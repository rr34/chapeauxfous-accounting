...
-- migration 0017: store-exact-reference-price-decimals
-- writer downtime: schedule maintenance; altering xrates takes a metadata lock.
-- deployment order: apply before exposing decimal reference-rate import tools.
-- recovery: restore the verified pre-migration backup if rollback is required.
-- Legacy reference ratios remain readable; transaction exchange ratios keep
-- their positive integer native-unit fields.

ALTER TABLE `xrates`
  COMMENT='Store exact transaction exchange ratios and timestamped reference prices. Transaction rows use positive integer native-unit ratios. Reference rows store their quoted positive decimal quantities exactly as supplied; legacy reference rows retain their original native-unit ratios. Reference prices do not change posted transaction values automatically.',
  MODIFY COLUMN `from_units` bigint(20) NULL DEFAULT NULL COMMENT 'Positive integer source-currency native units for transaction ratios and legacy reference ratios; null for a decimal-quoted reference price.',
  MODIFY COLUMN `to_units` bigint(20) NULL DEFAULT NULL COMMENT 'Positive integer target-currency native units for transaction ratios and legacy reference ratios; null for a decimal-quoted reference price.',
  ADD COLUMN `reference_from_decimal` varchar(256) NULL DEFAULT NULL COMMENT 'Exact positive source quantity as a decimal string in displayed currency units for a new reference price; null for transaction and legacy reference ratios.',
  ADD COLUMN `reference_to_decimal` varchar(256) NULL DEFAULT NULL COMMENT 'Exact positive target quantity as a decimal string in displayed currency units for a new reference price; null for transaction and legacy reference ratios.';
