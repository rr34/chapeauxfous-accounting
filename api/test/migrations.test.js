...
test("the repository migration ledger is valid and contiguous", () => {
  ...
  const artifactUploadMigration = migrations.find((migration) => migration.version === 14);
  assert.match(artifactUploadMigration.sql, /CREATE TABLE IF NOT EXISTS accounting_artifact_uploads/);
  assert.match(artifactUploadMigration.sql, /CREATE TABLE IF NOT EXISTS accounting_artifact_chunks/);
  assert.match(artifactUploadMigration.sql, /CREATE TABLE IF NOT EXISTS accounting_transaction_import_artifacts/);
  assert.match(artifactUploadMigration.sql, /chunk_bytes MEDIUMBLOB NOT NULL/);
  assert.match(artifactUploadMigration.sql, /ENUM\('receiving','complete'\)/);
  assert.doesNotMatch(artifactUploadMigration.sql, /GENERATED ALWAYS/i);
});
