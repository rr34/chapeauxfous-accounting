...
export const accountingCapabilityManifest = Object.freeze({
  ...
  capabilities: [
    ...
    },
    {
      ...
      guidance: "Use generic transaction import only when the source already describes complete balanced transactions with their account sides. Route every file listing movements for one named account through accounting.reconciliation and start_single_account_statement_import, including CSV files and requests to reach a known balance. Permanent deletion requires an MCP preview, one explicit confirmation, and the exact matching commit operation.",
      ...
      attachmentHints: [
        "A file listing movements into and out of one named account is a statement, even when it is called a transaction CSV or is expected to reach a known balance. Select accounting.reconciliation and call start_single_account_statement_import; do not create a generic transaction import job for those one-sided rows.",
        "For a generic complete-transaction file tied to a named account, select the accounting.accounts.active_paths context view to resolve its full path and currency once; use list_accounts only when current posted balances are needed.",
        "Fetch the authoritative canonical line-record JSON Schema only when the source already contains enough account sides to assemble complete balanced transactions.",
        ...
        "Group all lines of each generic transaction under one stable transaction_external_id before staging. Every generic transaction must already have its account sides. When the source provides only movements in one account, leave this workflow and use start_single_account_statement_import, which adds the designated suspense side. Preserve source account quantities exactly; valuation values are rounded to the valuation currency scale.",
        "For multi-account transaction assembly, when opening and closing known balances are available, use get_statement_reconciliation_context and analyze_statement_observations before staging, then prove that exact source quantities cover the required remaining account movement. The single-account statement workflow performs its own balance and duplicate analysis. Map source UTC time to transaction_at. Accounting automatically chooses the nearest owner-scoped reference rate in either currency direction and values each foreign line from its exact amount; a supplied source value is only a fallback when no rate exists. Keep exact cash proceeds as their own line and supply an existing expense fee_account_full_name for a cash-conversion residual; Accounting computes that fee. A missing counterpart is not a fee.",
    ...
    },
    {
      ...
      summary: "Import one-account transaction statements, check known balances, reconcile accounts, and import timestamped exchange-rate or market-price series.",
      ...
      attachmentHints: [
        ...
        "If a prior generic job turned every one-account statement row into TOO_FEW_LINE_ITEMS and UNBALANCED_TRANSACTION exceptions, stop repairing that job row by row and rerun the original attachment through start_single_account_statement_import and import_single_account_statement.",
